#!/usr/bin/env python3
"""
Lesson Creation Script - Updated for New Module Architecture

Creates a complete learning lesson using the new content_creator module.
...
Usage:
    ...
    python scripts/create_lesson.py \
        --lesson "React Native Views & Styles" \
...
from modules.content_creator.service import CreateLessonRequest
from modules.infrastructure.public import infrastructure_provider


async def main() -> None:
    """Main function to create lesson using new module architecture."""
    # Parse arguments
    parser = argparse.ArgumentParser(description="Create complete learning lesson using new module architecture")
    ...
    try:
        ...
        with infra.get_session_context() as db_session:
            ...
            request = CreateLessonRequest(title=args.lesson, core_concept=args.concept, source_material=source_material, user_level=args.level, domain=args.domain)

            # Generate complete lesson content
            if args.verbose:
                print("🔄 Generating complete lesson with AI-powered content...")
