...
export default function TopicContentPage() {
  ...
  console.log('🔍 [Learning Page] Mode params:', {
    modeParam,
    initialMode,
    useDuolingoFlow,
    viewMode,
    searchParamsMode: searchParams.get('mode'),
    searchParamsDuolingo: searchParams.get('duolingo')
  })

  const loadTopic = useCallback(async () => {
    try {
      console.log('🔍 [Learning Page] Starting to load topic:', topicId)
      setIsLoading(true)
      setError(null)

      // Use Duolingo service for optimized loading
      console.log('📡 [Learning Page] Calling duolingoLearningService.loadTopic...')
      const topicData = await duolingoLearningService.loadTopic(topicId, true)
      console.log('✅ [Learning Page] Duolingo service returned topic:', topicData?.title)
      setTopic(topicData)
      ...
      console.error('❌ [Learning Page] Duolingo service failed:', err)
      // Fallback to regular API if Duolingo service fails
      try {
        console.log('🔄 [Learning Page] Trying fallback with apiClient...')
        const topicData = await apiClient.getBiteSizedTopicDetail(topicId)
        console.log('✅ [Learning Page] Fallback succeeded:', topicData?.title)
        setTopic(topicData)
      } catch (fallbackErr) {
        console.error('❌ [Learning Page] Fallback also failed:', fallbackErr)
  ...
  useEffect(() => {
    console.log('🔍 [Learning Page] Topic ID:', topicId)
    console.log('🔍 [Learning Page] useEffect triggered')
    if (topicId) {
      console.log('🔍 [Learning Page] Topic ID exists, calling loadTopic')
      loadTopic()
    } else {
      console.log('❌ [Learning Page] No topic ID found!')
    }
  }, [topicId, loadTopic])
  ...
  if (viewMode === 'learning' && useDuolingoFlow) {
    console.log('🎯 [Learning Page] Rendering DuolingoLearningFlow with topic:', topic.title)
    return (
  ...
  if (viewMode === 'learning') {
    console.log('🎯 [Learning Page] Rendering regular learning flow with topic:', topic.title)
    return (
      <DuolingoLearningFlow
        topic={topic}
        onComplete={handleLearningComplete}
  ...
  console.log('🎯 [Learning Page] Rendering overview mode with viewMode:', viewMode, 'useDuolingoFlow:', useDuolingoFlow)
